#!/usr/bin/env bash
# Server-side serial timing pass over the C64 conditional matrix.
set -uo pipefail
...
failures="$DLB_ROOT/results/logs/conditional_benchmark_failures.tsv"
...
while IFS=$'\t' read -r task_id category model dataset steps sample_count seed environment adapter source provenance protocol conditioning_manifest conditioning_manifest_sha256 sample_dir metrics_path timing_path; do
  ...
  echo "CONDITIONAL BENCHMARK $task_id"
  "$SCRIPT_DIR/benchmark_conditional_one.sh" --model "$model" --dataset "$dataset" \
