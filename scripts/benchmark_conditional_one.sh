#!/usr/bin/env bash
# Run one server-side, registry-bound C64 conditional primary-latency benchmark.
set -u
...
if [[ -z "$model" || -z "$dataset" || -z "$steps" || -z "$precision" ]]; then
  usage; exit 2
fi
...
module_args=(--root "$DLB_ROOT" --models "$model" --datasets "$dataset" --steps "$steps" --seed "$seed" --precision "$precision" --generation-mode conditional_prefix)
