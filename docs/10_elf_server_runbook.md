...
**最新故障恢复：正式 XSum 在 216 条后遇到空条件，原始 WMT14 validation 的 BLEU 也出现明显下降。现在请优先执行 [条件输入诊断与恢复步骤](12_elf_condition_recovery.md)，暂停下文原始 validation/test 的条件实验命令。** 不重跑已完成的 OWT；保留旧结果，用作者 validation 全集建立条件质量与 timing，原始输入差异待诊断核实。
