...
**GPU 0、1 双卡最新入口：[自动核验并补齐质量实验](15_elf_gpu01_queue.md)。执行 `python scripts/run_elf_remaining.py --gpus 0 1 --plan` 查看待跑项，去掉 `--plan` 启动。请以文档15的命令为准，下面保留历史诊断背景。timing仍暂缓。**
...
**此前日志确认12个正式质量配置及一份XSum test输出。最新补跑安排见[双卡队列](15_elf_gpu01_queue.md)，队列会核验服务器实际结果，避免重复运行。timing继续暂缓。**

| 实验 | 已确认完成 | 尚需补齐 |
|---|---|---|
| OWT unconditional | 8/16/32/64 步各 1024 条，全部 valid=true | 主表补1/2/4/1024；64仅保留辅助结果 |
